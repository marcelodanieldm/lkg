...
export async function auditarSolicitud(formData) {
  ...
  try {
    ...
    if (data.informeHTML) {
      await guardarInforme(data.placeId, data.informeHTML, data.score, data.version || 'v1.0');
    }
    ...
    if (debeEnviar) {
      ...
      const cRes = await cronRoute(cronReq, { params: Promise.resolve({ tarea: 'aprobaciones' }) });
      if (cRes) {
