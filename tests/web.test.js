...
// ═══════════════════════════════════════════════════════════════════════
// Embudo simplificado — garantías de las 6 etapas verificables
// ═══════════════════════════════════════════════════════════════════════

test('una solicitud de dirección suprimida no crea lead', () => {
  // La función pedir_auditoria() en 004_solicitudes.sql detecta la baja pero
  // NO revierte opt_out ni inserta en leads. Solo deja una nota en la solicitud.
  // Este test verifica que el SQL de la función no contenga INSERT INTO leads.
  const sql = leer('supabase/migrations/004_solicitudes.sql');
  const fn = sql.slice(sql.indexOf('CREATE OR REPLACE FUNCTION pedir_auditoria'));
  assert.ok(!/INSERT\s+INTO\s+leads/i.test(fn),
    'pedir_auditoria() no debe insertar en leads — el formulario público solo crea solicitudes');
  assert.ok(/La baja NO se revirti/.test(sql),
    'la migración 004 debe documentar que una baja no se revierte aunque alguien complete el formulario');
});

test('opt_out sigue siendo irreversible después del embudo simplificado', () => {
  // El trigger proteger_opt_out fue reescrito en 012 para agregar los
  // timestamps de etapa. La garantía central no puede haberse perdido.
  const sql = leer('supabase/migrations/012_embudo_simplificado.sql');
  assert.ok(/opt_out = FALSE.*RAISE EXCEPTION.*irreversible/s.test(sql),
    'la migración 012 debe mantener el RAISE EXCEPTION que impide revertir opt_out');
  assert.ok(/NEW\.etapa\s*:=\s*'baja'/.test(sql),
    'el trigger de 012 debe forzar etapa = baja al hacer opt_out');
});

test('no se puede marcar un lead como perdido sin motivo', () => {
  // El trigger proteger_opt_out en 012 lanza una excepción si se intenta
  // poner etapa = 'perdido' sin motivo_perdida.
  const sql = leer('supabase/migrations/012_embudo_simplificado.sql');
  assert.ok(/NEW\.etapa\s*=\s*'perdido'[\s\S]*?motivo_perdida IS NULL/m.test(sql),
    'el trigger de 012 debe lanzar excepción al marcar perdido sin motivo');
  assert.ok(/obligatorio/.test(sql),
    'el mensaje de error del trigger debe decir que el motivo es obligatorio');
});

test('toda etapa activa tiene su plazo máximo en config', () => {
  // Los plazos no viven en el código sino en la tabla config para poder
  // ajustarlos sin desplegar. La migración 012 los inserta.
  const sql = leer('supabase/migrations/012_embudo_simplificado.sql');
  const etapasActivas = ['auditado', 'contactado', 'leyo', 'conversando', 'propuesta', 'cliente'];
  for (const e of etapasActivas) {
    assert.ok(sql.includes(`plazo_max_${e}`),
      `la migración 012 debe insertar la clave plazo_max_${e} en config`);
  }
});

test('la migración 012 es idempotente — no destruye sin salvaguarda', () => {
  // Idempotente significa: se puede correr dos veces sin error ni pérdida.
  // Esto excluye DROP TABLE, TRUNCATE y DELETE sin WHERE.
  const sql = leer('supabase/migrations/012_embudo_simplificado.sql');
  assert.ok(!/DROP\s+TABLE(?!\s+IF\s+EXISTS)/i.test(sql),
    'la migración 012 no debe hacer DROP TABLE sin IF EXISTS');
  assert.ok(!/TRUNCATE\s+/i.test(sql),
    'la migración 012 no debe hacer TRUNCATE');
  // Toda definición de objeto usa OR REPLACE, IF NOT EXISTS o bloque DO.
  assert.ok(/IF NOT EXISTS|OR REPLACE|DO \$\$/.test(sql),
    'la migración 012 debe usar IF NOT EXISTS / OR REPLACE para ser idempotente');
});
