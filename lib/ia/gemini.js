...
export async function agenteSugeridorMensaje({ negocio, score, potencial, hallazgos, categoria, ciudad }) {
  try {
    const r = await pedirJSON({
      ...
      sistema: 'Sos el especialista de comunicación comercial de Lokigi. Generás sugerencias breves, profesionales y amigables para acompañar auditorías.',
      prompt: `Generá un mensaje breve (2 a 3 oraciones) para enviarle al dueño de "${negocio}" (${categoria || 'comercio'} en ${ciudad || 'Argentina'}).
Puntaje de auditoría: ${score}/100. Potencial: ${potencial}/100.
Hallazgos principales: ${JSON.stringify((hallazgos || []).slice(0, 3))}

Requisitos:
- Tono profesional, amigable y respetuoso.
- NO uses palabras prohibidas como "garantizo", "primer puesto", "100% de éxito", "promesa", "sin riesgo".
- Resaltá una oportunidad de mejora útil y concreta para su negocio de forma constructiva.
