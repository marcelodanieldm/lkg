...
export const MODELOS = {
  rapido: 'gemini-3.5-flash-lite',  // 30 pedidos/min · clasificar, decidir zona
  bueno:  'gemini-3.6-flash',       // 15 pedidos/min · redactar
};
...
export async function agenteSugeridorMensaje({ negocio, score, potencial, hallazgos, categoria, ciudad }) {
  try {
    const r = await pedirJSON({
      nivel: 'bueno',
      temperatura: 0.4,
      esquema: ESQ_SUGERIDOR,
      sistema: 'Sos el especialista de comunicación comercial de Lokigi. Generás mensajes comerciales serios, cálidos y profesionales para presentar propuestas de valor a dueños de negocios.',
      prompt: `Generá un mensaje de apertura (2 a 3 oraciones) para el dueño de "${negocio}" (${categoria || 'comercio'} en ${ciudad || 'Argentina'}).
Puntaje de auditoría actual: ${score}/100. Potencial estimado: ${potencial}/100.
Hallazgos principales: ${JSON.stringify((hallazgos || []).slice(0, 3))}

Requisitos:
- Tono profesional, serio, cercano y cálido.
- Transmití entusiasmo por el potencial de crecimiento del perfil (de ${score} a ${potencial} pts) destacando el valor de captar más clientes locales.
- NO uses palabras prohibidas ni promesas engañosas ("garantizo", "primer puesto", "100% de éxito", "promesa", "sin riesgo").
- Devuelve únicamente el JSON con la propiedad "mensaje".`,
    });
    return r.salida?.mensaje || null;
  } catch {
    return null;
  }
}

const ESQ_ANALIZADOR_PERFIL = {
  type: 'object',
  properties: {
    categoriaPrimariaLabel: { type: 'string' },
    categoriaPrimaria: { type: 'string' },
    rating: { type: 'number' },
    cantidadResenas: { type: 'integer' },
    tasaRespuestaResenas: { type: 'number' },
    diasDesdeUltimaResena: { type: 'integer' },
    cantidadFotos: { type: 'integer' },
    descripcion: { type: 'string' },
    diasDesdeUltimaFoto: { type: 'integer' },
    ...
    tiposDeFoto: {
      ...
      properties: {
        ...
        producto: { type: 'boolean' },
        equipo: { type: 'boolean' },
      },
      required: ['fachada', 'interior', 'producto', 'equipo'],
    },
  },
  required: [
    'categoriaPrimariaLabel', 'categoriaPrimaria', 'rating',
    'cantidadResenas', 'tasaRespuestaResenas', 'diasDesdeUltimaResena',
    'cantidadFotos', 'tiposDeFoto'
  ],
};

export async function agenteAnalizadorPerfil({ negocio, direccion, ciudad, pais = 'Argentina' }) {
  try {
    const r = await pedirJSON({
      nivel: 'bueno',
      temperatura: 0.3,
      esquema: ESQ_ANALIZADOR_PERFIL,
      sistema: 'Sos el experto analista de auditorías de Google Business Profile de Lokigi. Analizás de forma realista y precisa el perfil público del negocio indicado.',
      prompt: `Analizá el perfil público del negocio "${negocio}" ubicado en "${[direccion, ciudad, pais].filter(Boolean).join(', ')}".
Estimá y analizá de forma realista sus datos públicos para auditoría de Google Maps:
- Categoría primaria precisa (etiqueta en español rioplatense y slug estándar de Places API).
- Rating promedio típico o real (entre 3.5 y 5.0) y cantidad de reseñas acorde a su rubro/tamaño/zona.
- Tasa de respuesta a reseñas (entre 0.0 y 1.0) y días transcurridos desde la última reseña.
- Cantidad de fotos, tipos de fotos presentes y días desde la última foto.
- Atributos clave y lista de servicios que ofrece este negocio.

Devuelve únicamente el JSON estructurado según el esquema.`,
    });
    return r.salida || null;
  } catch {
    return null;
  }
}
