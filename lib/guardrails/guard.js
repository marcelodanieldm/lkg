...
export function cupoDelDia(canal, cfg) {
  ...
  if (!cfg.inicioCalentamiento) return Math.min(base, 10);
  const dias = Math.floor((Date.now() - new Date(cfg.inicioCalentamiento).getTime()) / 86_400_000);
  if (dias < 7) return Math.min(base, 10);
  if (dias < 14) return Math.min(base, 18);
  if (dias < 21) return Math.min(base, 25);
  if (dias < 28) return Math.min(base, 35);
  return base;
}
