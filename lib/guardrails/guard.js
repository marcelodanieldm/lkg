...
export const PROHIBIDO = [
  ...
  { re: B('(?:te\\s+posicionaremos|te\\s+ubicaremos|quedarás|quedaras|te\\s+ponemos|alcanzarás|alcanzaras)\\s+(?:en\\s+el|en\\s+la|1er|1º|primer|segundo|tercer|posicion|puesto|lugar)\\s+(?:de|en)?\\s*(?:google|maps|buscadores|búsqueda|busqueda)'), que: 'promesa de posición' },
  { re: B('(?:prometer|garantizar)(?:te)?\\s+(?:un\\s+puesto|una\\s+posicion|un\\s+lugar|el\\s+primer\\s+lugar)\\s+en\\s+(?:google|maps|buscadores)'), que: 'promesa de posición' },
  { re: B('(?:posicionamiento|posición|puesto|lugar)\\s+(?:garantizado|asegurado)\\s+en\\s+(?:google|maps)'), que: 'promesa de posición' },
...
const REGLAS = [
  ...
  },
  {
    ...
    async check(i, ctx) {
      ...
      if (e === 'cliente' && i.tipo === 'prospeccion')
 ...
 */
export async function evaluar(intento, ctxOverride = null) {
  const id = huella(intento);
  let ctx;
  try {
    ctx = ctxOverride || await contexto(intento);
  } catch (e) {
