...
export async function agendarLlamada({ negocio, email, inicio, fin, notas, calendarId = 'primary' }) {
  const r = await llamar(
    `${CAL}/calendars/${encodeURIComponent(calendarId)}/events?conferenceDataVersion=1&sendUpdates=all`,
    {
      method: 'POST',
      body: JSON.stringify({
