...
/** Busca negocios por texto + ubicación. Para armar listas de prospección. */
export async function buscarNegocios({ consulta, lat, lng, radioMetros = 5000, max = 20, idioma = 'es-419', region = 'AR' }) {
  const res = await fetch(`${BASE}/places:searchText`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'X-Goog-Api-Key': requireKey(),
      // fieldMask mínimo => se factura en el escalón barato de Text Search
      'X-Goog-FieldMask': 'places.id,places.displayName,places.formattedAddress,places.primaryTypeDisplayName,places.rating,places.userRatingCount,places.websiteUri,places.googleMapsUri,places.location',
    },
    body: JSON.stringify({
      textQuery: consulta,
      languageCode: idioma,
      ...
      ...(lat && lng ? { locationBias: { circle: { center: { latitude: lat, longitude: lng }, radius: radioMetros } } } : {}),
    }),
  });
