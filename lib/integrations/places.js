...
export async function buscarNegocios({ consulta, lat, lng, radioMetros = 5000, max = 20, idioma = 'es-419', region = 'AR' }) {
  const res = await fetch(`${BASE}/places:searchText`, {
    ...
    headers: {
      ...
      'X-Goog-FieldMask': 'places.id,places.displayName,places.formattedAddress,places.primaryTypeDisplayName,places.rating,places.userRatingCount,places.websiteUri,places.googleMapsUri',
    },
    body: JSON.stringify({
      textQuery: consulta,
      languageCode: idioma,
