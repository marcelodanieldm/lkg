...
import { upgradeInformeHTML } from '../core/report.js';
...
/**
 * Eleva un informe de auditoría previamente congelado agregándole el bloque
 * completo de competencia procedente del barrido.
 *
 * INVARIANTE CRÍTICO:
 * El puntaje, dimensiones y hallazgos originales NO cambian (byte a byte).
 * Solo se sustituye el bloque de muestra por el bloque comparativo completo.
 */
export async function recongelarInformeConBarrido(leadId, comparacion, { fuenteDatos = null } = {}) {
  const db = fuenteDatos;
  if (!db) return { ok: false, error: 'sin_fuente_datos' };

  let htmlOriginal = null;
  if (typeof db.informeHTML === 'function') {
    htmlOriginal = await db.informeHTML(leadId).catch(() => null);
  } else if (typeof db.uno === 'function') {
    const inf = await db.uno('Informes', i => i.lead_id === leadId).catch(() => null);
    htmlOriginal = inf?.html || null;
  }

  if (!htmlOriginal) return { ok: false, error: 'informe_no_encontrado' };

  const nuevoHtml = upgradeInformeHTML(htmlOriginal, comparacion);

  if (typeof db.recongelarInforme === 'function') {
    await db.recongelarInforme(leadId, nuevoHtml);
  } else if (typeof db.actualizar === 'function') {
    await db.actualizar('Informes', { lead_id: leadId, html: nuevoHtml, actualizado_en: new Date().toISOString() }, 'lead_id');
  }

  return { ok: true, leadId, html: nuevoHtml };
}
