...
export function bloqueComparacionCompleto(c) {
  ...
  const ad = c.anillosDistancia;
  const ch = c.coberturaHoraria;
  ...
  const bloqueAnillos = (ad && ad.sePublica) ? `
  <div style="background:#f8fafc;border:1px solid #cbd5e1;border-left:4px solid #64748b;border-radius:8px;padding:12px 14px;margin-top:12px;font-size:14px;color:#334155;">
    <b style="color:#1e293b">Anillos de distancia (proximidad real):</b>
    <ul style="margin:6px 0 0;padding-left:18px">
      ${ad.anillo500m?.texto ? `<li style="margin-bottom:4px">${esc(ad.anillo500m.texto)}</li>` : ''}
      ${ad.anillo1500m?.texto ? `<li style="margin-bottom:4px">${esc(ad.anillo1500m.texto)}</li>` : ''}
      ${ad.anillo4000m?.texto ? `<li style="margin-bottom:4px">${esc(ad.anillo4000m.texto)}</li>` : ''}
    </ul>
    ${ad.diagnosticoContradiccion ? `<p style="margin:8px 0 0;font-weight:600;color:#0f172a"><b>Diagnóstico:</b> ${esc(ad.diagnosticoContradiccion)}</p>` : ''}
  </div>` : '';

  const bloqueHorarios = (ch && ch.sePublica && ch.franjasDestacadas?.length) ? `
  <div style="background:#fdf4ff;border:1px solid #f5d0fe;border-left:4px solid #c084fc;border-radius:8px;padding:12px 14px;margin-top:12px;font-size:14px;color:#701a75;">
    <b style="color:#86198f">Cobertura horaria de la zona:</b>
    <ul style="margin:6px 0 0;padding-left:18px">
      ${ch.franjasDestacadas.map(f => `<li style="margin-bottom:4px">${esc(f.texto)}</li>`).join('')}
    </ul>
  </div>` : '';
