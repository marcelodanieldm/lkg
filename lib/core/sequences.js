 ...
 *  - Máximo 3 toques por correo. Si no hay respuesta, se cierra el ciclo y se avisa que se cierra.
 *  - Sin promesas de posición ni de resultado numérico garantizado.
 *  - Voseo Rioplatense 100%: sin mezclas de "usted" ni "su perfil".
 *
 * NOTA DE DISEÑO SOBRE WHATSAPP EN PROSPECCIÓN EN FRÍO:
 * WhatsApp NUNCA inicia conversaciones en frío. Un WhatsApp enviado a un número
 * obtenido de Google Maps sin consentimiento previo se clasifica como categoría
 * MARKETING en Meta y es la causa directa por la que Meta sanciona y bloquea el
 * número emisor. WhatsApp se utiliza únicamente cuando el prospecto responde la
 * secuencia por correo (abriendo la ventana conversacional sin requerir plantillas aprobadas).
 *
 ...
 *  competenciaDelta, urlInforme, agencia, remitente, urlBaja, zona
 */
export const SECUENCIA_PROSPECCION = [
  {
    ...
    asunto: 'Tu perfil en Google Maps: {{score}}/100',
    cuerpo: `Hola{{saludoContacto}},

Soy {{remitente}}, de {{agencia}}. Estuve revisando el perfil de {{negocio}} en Google Maps y trabajo optimizando perfiles para negocios de {{zona}}.

Analicé el perfil con la misma grilla que uso con mis clientes: 25 puntos de control sobre datos del perfil, reseñas, fotos, actividad y conversión. El resultado dio {{score}} sobre 100 ({{banda}}).
...
Sumé un dato al informe de {{negocio}}: audité con la misma grilla los perfiles que hoy aparecen arriba tuyo cuando alguien busca tu categoría en {{zona}}.
...
El informe de {{negocio}} queda disponible en el mismo link ({{urlInforme}}) por 90 días, para que lo uses con quien quieras, conmigo o no.
