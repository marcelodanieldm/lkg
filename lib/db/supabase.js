...
const TABLA = {
  ...
  Solicitudes: 'solicitudes', Informes: 'informes', Propuestas: 'propuestas',
};

const nombreTabla = (pestana) => TABLA[pestana] || String(pestana).toLowerCase();
...
export async function leer(pestana, { sinCache = false, limite = 5000 } = {}) {
  const tabla = nombreTabla(pestana);
...
export async function agregar(pestana, objetos, { ignorarDuplicados = true } = {}) {
  const lista = (Array.isArray(objetos) ? objetos : [objetos]).map(o => desnormalizar(pestana, o));
  if (!lista.length) return { agregadas: 0 };

  // Las tablas con reglas/triggers de Postgres ('Mensajes', 'Supresiones', 'Aprobaciones', 'Bitacora', 'Solicitudes')
  // no admiten la cláusula ON CONFLICT (resolution=ignore-duplicates en PostgREST) y lanzan error 400 (0A000).
  const sinOnConflict = ['Mensajes', 'Supresiones', 'Aprobaciones', 'Bitacora', 'Solicitudes', 'Leads', 'Propuestas', 'Informes'].includes(pestana) || !ignorarDuplicados;
  const preferHeader = sinOnConflict ? 'return=minimal' : 'return=minimal,resolution=ignore-duplicates';

  await rest(nombreTabla(pestana), {
    method: 'POST',
...
export async function actualizar(pestana, cambios, clave = 'id') {
  ...
  for (const c of lista) {
    ...
    await rest(`${nombreTabla(pestana)}?${clave}=eq.${encodeURIComponent(valor)}`, {
      method: 'PATCH',
...
/** Inserta o actualiza en una sola llamada. Si Postgres rechaza ON CONFLICT por triggers, recurre a fallback. */
export async function upsert(pestana, obj, clave = 'id') {
  const o = desnormalizar(pestana, obj);
  try {
    await rest(`${nombreTabla(pestana)}?on_conflict=${clave}`, {
      method: 'POST',
    ...
    if (valor !== undefined && valor !== null) {
      const existe = await rest(`${nombreTabla(pestana)}?${clave}=eq.${encodeURIComponent(valor)}&select=${clave}`).catch(() => []);
      if (Array.isArray(existe) && existe.length > 0) {
        await actualizar(pestana, obj, clave);
      } else {
        await agregar(pestana, obj, { ignorarDuplicados: true });
      }
    } else {
      throw err;
    }
...
export async function guardarConfig(clave, valor, nota) {
  await upsert('Config', { clave, valor: String(valor), ...(nota ? { nota } : {}) }, 'clave');
  _cfg = null;
}
...
export const guardarInforme = (leadId, html, score, version) =>
  upsert('Informes', { lead_id: leadId, html, score, version_rubrica: version }, 'lead_id');

/** Recongela un informe existente con el HTML actualizado. El disparador de Postgres guarda el anterior en html_anterior. */
export const recongelarInforme = (leadId, nuevoHtml) =>
  rest(`informes?lead_id=eq.${encodeURIComponent(leadId)}`, {
    method: 'PATCH',
    headers: { Prefer: 'return=minimal' },
    body: JSON.stringify({ html: nuevoHtml, actualizado_en: new Date().toISOString() }),
  });

/** Obtiene la versión anterior del HTML congelado (resguardada antes de recongelar). */
export const obtenerInformeAnterior = async (leadId) => {
  const r = await rest(`informes?select=html_anterior&lead_id=eq.${encodeURIComponent(leadId)}`).catch(() => []);
  return r?.[0]?.html_anterior ?? null;
};
