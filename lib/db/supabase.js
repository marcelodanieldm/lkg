...
export async function agregar(pestana, objetos, { ignorarDuplicados = true } = {}) {
  ...
  // Las tablas con reglas de Postgres ('mensajes', 'supresiones') no admiten la cláusula
  // ON CONFLICT (resolution=ignore-duplicates en PostgREST) y lanzan error 400 (0A000).
  const sinOnConflict = ['Mensajes', 'Supresiones'].includes(pestana) || !ignorarDuplicados;
